...
> **Definition.**  $$\frac{d^2y}{dx^2} = \big(y'(x)\big)' = \frac{d\big(y'\big)/dt}{dx/dt} = \frac{\dfrac{d}{dt}\!\left(\dfrac{y'(t)}{x'(t)}\right)}{x'(t)} .$$
...
> **Example.** For the curve above, find the values of $t$ for which $C$ is concave up
> and concave down.
...
> **Caution.** Both divisions are by $x'(t)$. Students who write $y''(t)/x''(t)$ get
...
> **Theorem.** Let $C:\ x = f(t),\ y = g(t),\ \alpha \le t \le \beta$, such that
...
> **Note — length versus distance.** If the particle moves along $C$ *more than once*
...
> $C:\ x = 3\cos t,\ y = 3\sin t,\ 0 \le t \le 2\pi$ — traced once, counter-clockwise from
> $(3, 0)$.
...
> **Definition.** With the same hypotheses as the length proposition:
> 2. if $C$ is rotated about the **$x$-axis**, the area of the resulting surface is
>  $$S = 2\pi \int_\alpha^\beta y(t)\,\sqrt{x'(t)^2 + y'(t)^2}\,dt, \qquad y(t) \ge 0 ;$$
> 3. if $C$ is rotated about the **$y$-axis**,
...
> **Caution.** Using the full circle $0 \le t \le 2\pi$ about the $x$-axis is wrong
...
> **Definition.** A **loop** is a curve that crosses itself: there are two different
...
> **Example.** Sketch $C:\ x(t) = 3t - t^3,\ y(t) = 3 - t^2,\ t \in \mathbb{R}$.
...
> **Key insight.** A loop is not a new kind of object — it is the ordinary sketching
