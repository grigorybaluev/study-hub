...
> **Definition.**  $$\Sigma^* = \text{the set of all possible strings over } \Sigma, \qquad \Sigma^+ = \Sigma^* - \{\lambda\}$$
> For $\Sigma = \{a,b\}$: $\Sigma^* = \{\lambda, a, b, aa, ab, ba, bb, aaa, aab, \dots\}$ and $\Sigma^+ = \{a, b, aa, ab, ba, bb, aaa, aab, \dots\}$.

> **Key insight.** Strings are finite sequences; λ is the empty one (length 0, neutral for concatenation). Σ* is the infinite set of all strings over Σ — every language will be a subset of it.
...
> **Note — Two meanings of $|\cdot|$.** $|w|$ is the *length* of a string; $|L|$ is the *size* of a set. So $|\{\lambda\}| = 1$ while $|\lambda| = 0$; $|\varnothing| = 0$. Note also $\varnothing \ne \{\lambda\}$: the empty language has no strings, the second has one (the empty string).
...
> **Definition.**  $$L^R = \{\, w^R : w \in L \,\}, \qquad L_1 L_2 = \{\, xy : x \in L_1,\ y \in L_2 \,\}$$
...
> **Definition.**  $$L^n = \underbrace{L L \cdots L}_{n}, \qquad L^0 = \{\lambda\}$$ $$L^* = L^0 \cup L^1 \cup L^2 \cup \cdots, \qquad L^+ = L^1 \cup L^2 \cup \cdots$$
...
> - $\{a, bb\}^* = \{\lambda,\ a, bb,\ aa, abb, bba, bbbb,\ aaa, aabb, abba, abbbb, \dots\}$.
> - $\{a, bb\}^+ = \{a, bb,\ aa, abb, bba, bbbb,\ aaa, aabb, abba, abbbb, \dots\}$.
> **Question from the slides:** when is $L^+ = L^* - \{\lambda\}$? Exactly when $\lambda \notin L$ — if $\lambda \in L$ then $\lambda \in L^1 \subseteq L^+$ as well.

> **Key insight.** A language is a set of strings, so ∪, ∩, − and complement come for free; the new operations are concatenation L₁L₂, powers Lⁿ, and the closures L* (all finite concatenations, including λ) and L⁺ (at least one factor).
...
A **derivation** of “the dog walks”:
 $$\begin{gathered} \text{sentence} \Rightarrow \text{noun\_phrase predicate} \Rightarrow \text{noun\_phrase verb} \Rightarrow \text{article noun verb} \\[4pt] \Rightarrow \text{the noun verb} \Rightarrow \text{the dog verb} \Rightarrow \text{the dog walks} \end{gathered}$$
...
> **Example.**
> Derivation of $ab$: $\; S \Rightarrow aSb \Rightarrow ab$.
> Derivation of $aabb$: $\; S \Rightarrow aSb \Rightarrow aaSbb \Rightarrow aabb$.
> Other derivations: $S \Rightarrow aSb \Rightarrow aaSbb \Rightarrow aaaSbbb \Rightarrow aaabbb$, and $S \Rightarrow \dots \Rightarrow aaaabbbb$. The language of the grammar is
>  $$L = \{\, a^n b^n : n \ge 0 \,\}.$$
...
> **Definition.**
> **Language of a grammar.** For a grammar $G$ with start variable $S$, $$L(G) = \{\, w : S \overset{*}{\Rightarrow} w \,\}, \quad w \text{ a string of terminals}.$$

### Second grammar example

> **Example.**
> $G$: $\; S \to Ab, \quad A \to aAb, \quad A \to \lambda$. Derivations:
>  $$S \Rightarrow Ab \Rightarrow b, \qquad S \Rightarrow Ab \Rightarrow aAbb \Rightarrow abb, \qquad S \Rightarrow Ab \Rightarrow aAbb \Rightarrow aaAbbb \Rightarrow aabbb$$ $$S \Rightarrow Ab \Rightarrow aAbb \Rightarrow aaAbbb \Rightarrow aaaAbbbb \Rightarrow aaaaAbbbbb \Rightarrow aaaabbbbb$$
> In general $S \overset{*}{\Rightarrow} a^n b^n b$, so $L(G) = \{\, a^n b^n b : n \ge 0 \,\}$.
> **A convenient notation:** rules with the same left-hand side are combined with $|$: $\; A \to aAb \mid \lambda$, $\; \text{article} \to a \mid \text{the}$.

> **Key insight.** A derivation is a sequence of rewriting steps S ⇒ … ⇒ w, each replacing one variable by the right-hand side of a rule; L(G) is the set of all terminal strings reachable this way. The same language can have many grammars — and many derivations of the same string.
