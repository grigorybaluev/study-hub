...
::::example[$\ln x$ has no obvious $dv$]
Find $\displaystyle\int \ln x\,dx$.

:::solution
Take $u = \ln x$ and $dv = dx$. Then $du = dx/x$, $v = x$, and

$$
\int \ln x\,dx = x\ln x - \int x\cdot\frac{dx}{x} = x\ln x - x + C .
$$

The same trick handles $\int \tan^{-1}x\,dx$ and $\int \sin^{-1}x\,dx$.
:::
::::

::::example[Twice, then solve]
Find $I = \displaystyle\int e^x\cos x\,dx$.

:::solution
With $u = e^x$, $dv = \cos x\,dx$: $I = e^x\sin x - \int e^x\sin x\,dx$. Apply parts again to
the new integral, with $u = e^x$, $dv = \sin x\,dx$:

$$
\int e^x\sin x\,dx = -e^x\cos x + \int e^x\cos x\,dx = -e^x\cos x + I .
$$

Hence $I = e^x\sin x + e^x\cos x - I$, so $I = \tfrac12 e^x(\sin x + \cos x) + C$. Keep the
*same* kind of $u$ both times, or the second step just undoes the first.
:::
::::
...
$$
\int_0^{1} x e^{-x}\,dx = \Big[-x e^{-x}\Big]_0^1 + \int_0^1 e^{-x}\,dx = -e^{-1} + \Big[-e^{-x}\Big]_0^1 = 1 - \frac{2}{e} .
$$
:::
::::
...
:::insight
The formula trades $\int u\,dv$ for $\int v\,du$. It is only useful if the
