...
:::definition[Sequence]
A **sequence** is an ordered list of infinitely many real numbers $a_1, a_2, a_3, \dots$, written $\{a_n\}$ or $\{a_n\}_{n=1}^{\infty}$. Equivalently it is a function $f$ on the integers from some starting point onward, with $a_n = f(n)$.
:::
...
$\{a_n\}$ is **bounded above** by $M$ if $a_n \le M$ for all $n$, **bounded below** by $L$ if $a_n \ge L$ for all $n$, and **bounded** if both — then $|a_n| \le K = \max\{|L|, |M|\}$. The smallest upper bound is the **least upper bound**; the largest lower bound the **greatest lower bound**. It is **unbounded** if for every $A > 0$ some term has $|a_n| > A$.
:::

:::definition[Monotonic and alternating sequences]
$\{a_n\}$ is **increasing** if $a_{n+1} > a_n$ for all $n$, **decreasing** if $a_{n+1} < a_n$ for all $n$, **monotonic** if either. It is **alternating** if $a_n a_{n+1} < 0$ for all $n$ — the signs flip every step.
:::

:::theorem[Monotonicity from the derivative]
If $a_n = f(n)$ for a differentiable $f$ on $[1, \infty)$, then $\{a_n\}$ is decreasing when $f'(x) < 0$ there and increasing when $f'(x) > 0$.
:::

Two other ways to test monotonicity: check the sign of $a_{n+1} - a_n$, or, for positive
terms, compare $a_{n+1}/a_n$ with $1$.

::::example[Monotonic and bounded]
Is $a_n = \dfrac{n}{n^2 + 1}$ monotonic? Is it bounded?
...
so the sequence is decreasing from $n = 1$ on. It is bounded below by $0$ and above by
$a_1 = \tfrac12$.
:::
::::
...
$\{a_n\}$ **converges to $L$**, written $\lim_{n \to \infty} a_n = L$, if for every
...
Otherwise it **diverges**: to $\infty$ if $a_n \to \infty$, to $-\infty$ if $a_n \to -\infty$,
or simply diverges if there is no limit at all.
:::

:::theorem[Limits of functions give limits of sequences]
...
::::theorem[Absolute value tending to zero]
If $\lim |a_n| = 0$ then $\lim a_n = 0$.

:::proof
Squeeze $a_n$ between $-|a_n|$ and $|a_n|$, both of which tend to $0$.
:::
::::

:::theorem[Continuous functions of limits]
If $\lim a_n = L$ and $f$ is continuous at $L$ (and defined at every $a_n$), then $\lim f(a_n) = f(L)$ — limits pass through continuous functions.
:::

::::example[Three limits]
Find the limits, if they exist, of $a_n = \dfrac{(-1)^n}{n}$, $\ a_n = r^n$ and $\ a_n = \dfrac{\ln n}{n}$.

:::solution
- $|(-1)^n/n| = 1/n \to 0$, so $a_n \to 0$.
- $r^n$ converges to $0$ for $|r| < 1$, to $1$ for $r = 1$, and diverges otherwise.
- By l'Hôpital on $\ln x / x$, the limit of $\ln n / n$ is $0$.
:::
::::
...
If $\{a_n\}$ is bounded above and (eventually) increasing, it converges; if bounded below and (eventually) decreasing, it converges.
:::
...
:::solution
Induction shows $a_n < 2$ and $a_{n+1} > a_n$, so by monotone convergence the sequence
converges to some $L$. Passing to the limit in the recursion, $L = \sqrt{2 + L}$, so
$L^2 - L - 2 = 0$ and $L = 2$ (the root $-1$ is impossible for positive terms).
:::
::::
