...
> **Definition.** A function $F$ is an **antiderivative** of $f$ on an interval $I$ if $F'(x) = f(x)$ for every $x \in I$.

> **Theorem.** If $F$ is one antiderivative of $f$ on an interval $I$, then every
> antiderivative of $f$ on $I$ has the form $F(x) + C$ for some constant $C$.
...
> **Key insight.** Antidifferentiation is *not* unique, but it is unique up to a
> constant. Geometrically the family $F + C$ is one curve slid up and down: every member
> has the same slope $f(x)$ at every $x$, so the curves never cross.
...
> **Caution.** The power rule fails at $n = -1$: $x^{-1}$ has antiderivative $\ln|x|$, not
...
> **Example.** Find all antiderivatives of $f(x) = 3x^2 - 2\sin 2x + \dfrac{4}{x}$.
> Term by term: $x^3 + \cos 2x + 4\ln|x| + C$. Check by differentiating:
> $3x^2 - 2\sin 2x + 4/x$ ✓. One constant $C$ suffices for the whole sum — three separate
> constants would just add up to one.

> **Example — an initial-value problem.** A particle has acceleration $a(t) = 6t$,
> velocity $v(0) = 2$ and position $s(0) = 1$. Then $v(t) = 3t^2 + C_1$, and $v(0) = 2$ gives
> $C_1 = 2$; next $s(t) = t^3 + 2t + C_2$, and $s(0) = 1$ gives $C_2 = 1$. Each
> antidifferentiation introduces one constant, and each initial condition fixes one.

> **Note.** Not every function has an *elementary* antiderivative: $e^{-x^2}$, $\sin(x^2)$
