...
$|A| = |B|$ iff there is a bijection $A \to B$. If there is a surjection
...
::::example[The odd positive integers]
Show that the odd positive integers $O$ are countable.

:::solution
$f(n) = 2n - 1$ is a bijection $\mathbb Z^+ \to O$. Injective: $2n - 1 = 2m - 1 \Rightarrow n = m$.
Surjective: for odd $t$, $k = (t + 1)/2$ is a positive integer with $f(k) = t$.
:::
::::
...
:::solution
$f(n) = 2n + 1$ for $n \ge 0$ and $f(n) = -2(n + 1)$ for $n < 0$ is a bijection
$\mathbb Z \to \mathbb Z^+$: non-negatives go to the odd numbers, negatives to the even ones.
:::
::::

::::example[Pairs of positive integers]
Show that $\mathbb Z^+ \times \mathbb Z^+$ is countable.

:::solution
Lay the pairs out in a table and walk the diagonals:
$(1,1), (1,2), (2,1), (1,3), (2,2), (3,1), \dots$ — pairs in increasing order of $i + j$, and
lexicographic within a diagonal. Mapping each pair to its position is a bijection with
$\mathbb Z^+$.
:::
::::

::::example[The positive rationals]
Show that $\mathbb Q^+$ is countable.

:::solution
Treat $i/j$ as the pair $(i, j)$, list as above, and skip any fraction equal to one already
listed.
:::
::::

:::proposition[Closure of countability]
...
:::proof
By diagonalization. Suppose $\mathbb R$ were countable; then so is $[0, 1]$, listed
...
:::insight
"Same size" for infinite sets means "there is a bijection", and it
