...
> **Definition.** A **binary relation** from $A$ to $B$ is any subset $R \subseteq A \times B$.
...
> **Definition.** For $R$ from $A$ to $B$ and $S$ from $B$ to $C$, the **composite**
...
**Powers.** For $R$ on $A$, $R^1 = R$ and $R^n = R^{n-1} \circ R$; $a\,R^n\,b$ iff the graph
has a path of exactly $n$ arcs from $a$ to $b$.

> **Theorem.** $R$ is transitive iff $R^n \subseteq R$ for all $n \ge 1$.
...
> **Theorem.** On a finite set of $n$ elements, $R^* = R \cup R^2 \cup \cdots \cup R^n$.
>
> *Why.* A shortest path from $a$ to $b$ never revisits a node, so it has at most $n$ arcs.

> **Key insight.** A relation is a set of pairs, so it can be pictured as a directed graph
