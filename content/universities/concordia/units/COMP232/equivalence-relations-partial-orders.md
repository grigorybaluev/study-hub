...
> **Definition.** A relation on $A$ is a **partial order** if it is reflexive, antisymmetric
> and transitive; $(A, R)$ is then a **poset**. We write $a \preccurlyeq b$ for $(a, b) \in R$ and
> $a \prec b$ when also $a \ne b$.

$\le$ on numbers and $\subseteq$ on sets are partial orders. Elements $a, b$ are
**comparable** if $a \preccurlyeq b$ or $b \preccurlyeq a$; a poset in which every pair is
comparable is a **total order** ($\le$ on $\mathbb N$; not $\subseteq$).

**Hasse diagram.** The graph of a poset is cluttered by the loops (reflexivity) and the
shortcut arcs (transitivity). Remove both, and draw $b$ above $a$ whenever $a \prec b$; the
remaining lines are the diagram. For $(\mathcal P(\{1,2,3\}), \subseteq)$ it is a cube with
...
**Lexicographic order.** A partial order on letters extends to words as dictionary order:
on $A \times B$, $(a_1, b_1) \prec (a_2, b_2)$ iff $a_1 \prec a_2$, or $a_1 = a_2$ and
$b_1 \prec b_2$; on strings, compare position by position and let a proper prefix come first.
...
> **Example.** In the Hasse diagram of the COMP core courses, the courses with no
> prerequisite are the minimal elements, the courses nothing depends on are maximal, and
> the least upper bound of $\{$COMP 232, COMP 249$\}$ is the first course requiring both.

> **Key insight.** Reflexive + transitive plus *symmetric* gives an equivalence — a way to
