...
> **Definition.** A **theorem** is a statement that can be proved true. An **axiom** is a
...
Throughout, the integer definitions are: $m$ is **even** iff $m = 2i$ for some integer $i$;
**odd** iff $m = 2i + 1$.
...
> **Example.** If $n$ is even then $n^2$ is even. Suppose $n = 2i$. Then
> $n^2 = 4i^2 = 2(2i^2)$, and $2i^2$ is an integer, so $n^2$ is even.
...
> **Example.** If $n^2$ is even then $n$ is even. Contrapositive: if $n$ is odd then $n^2$
> is odd. Suppose $n = 2i + 1$; then $n^2 = 4i^2 + 4i + 1 = 2(2i^2 + 2i) + 1$, odd.

An "if and only if" needs both directions: $p \Rightarrow q$ *and* $q \Rightarrow p$. The two
examples together prove "$n$ is even $\iff n^2$ is even".
...
> **Example.** $\sqrt 2$ is irrational. Suppose $\sqrt 2 = a/b$ with $a, b$ integers having no
> common factor. Then $2b^2 = a^2$, so $a^2$ is even, so $a$ is even, $a = 2i$; then
> $2b^2 = 4i^2$, $b^2 = 2i^2$, so $b$ is even too — a common factor $2$, contradiction.

> **Example.** If $3n + 2$ is odd then $n$ is odd. Assume $3n + 2$ odd and $n$ even,
> $n = 2k$. Then $3n + 2 = 6k + 2 = 2(3k + 1)$ is even — contradicting the assumption.
...
> **Example.** $\max(x, y) + \min(x, y) = x + y$. Case $x \ge y$: the sum is $x + y$. Case
> $x < y$: the sum is $y + x$. Either way the identity holds.

> **Key insight.** Choosing the method is choosing which statement to *assume*: direct
